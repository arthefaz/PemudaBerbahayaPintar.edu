...
export const KineticTypographyLoader = () => {
    ...
    const words = ["MEMUAT", "MENYIAPKAN", "MEMBANGUN"];
    ...
    useEffect(() => {
        ...
        function animateWord() {
            ...
            const chars = word.split('').map((char, index) => {
                ...
                span.className = 'char font-display';
                // Need to add space handling
                if (char === ' ') {
                    span.innerHTML = '&nbsp;';
                } else {
    ...
    return (
        <div className="loader-container flex items-center justify-center min-h-[60vh] perspective-[1000px]">
            <h1 ref={loaderTextRef} className="text-5xl sm:text-7xl lg:text-9xl font-extrabold text-primary tracking-widest whitespace-nowrap drop-shadow-xl" style={{ transformStyle: 'preserve-3d' }}>
            </h1>
        </div>
    );
};
