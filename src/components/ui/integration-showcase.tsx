      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 items-start gap-x-12 gap-y-10 lg:grid-cols-2">
          ...
          <div className="flex items-center justify-center lg:justify-center">
            <img 
              src={illustrationSrc} 
              alt={illustrationAlt} 
              className="w-64 h-auto object-contain rounded-2xl shadow-xl border border-border/50"
            />
          </div>
        </div>
        ...
        >
          {integrations.map((item) => (
            <motion.div key={item.name} variants={itemVariants} className="flex items-start space-x-4 p-4 rounded-xl hover:bg-muted/50 transition-colors">
              <div className="flex-shrink-0 bg-background p-2 rounded-lg shadow-sm border border-border/50">
                <img 
                  src={item.iconSrc} 
                  alt={`${item.name} logo`} 
